...
%% @doc Implement required exchange behaviors for the gearman_http_exchange
%% @end

-module(gearman_http_exchange).

-include_lib("rabbit_common/include/rabbit.hrl").

-define(X_TYPE, <<"x-gearman-http">>).
-define(X_DESC, <<"Gearman HTTP Exchange">>).
...
route(X, Delivery) ->
  case gearman_http_lib:post(X, Delivery) of
    ok ->
...
validate(X) ->
  case gearman_http_lib:validate(X) of
    ok ->
      ok;
    {error, Error} ->
